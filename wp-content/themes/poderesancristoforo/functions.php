	...
	function start_el(&$output, $item, $depth=0, $args=array(), $id = 0) {
		...
		if ($depth == 0) {
			...
			if ($item->ID == 8 || $item->ID == 9 || $item->ID == 10) {
				$class_names = ' class= "nav-item border-item d-none d-md-flex ' . esc_attr( $class_names ) . '"';
			} else {
...
function send_my_awesome_form(){
    ...
    $send_to = ['lorenzo.zonin@poderesancristoforo.it'];
...
function send_my_collect_mail_form(){
    ...
    //set the form headers
    $headers = 'From: Podere San Cristoforo - Contact Request' . "\r\n";
...
function request_visit_mail_form(){
    ...
    $form['mi_name'] = $_POST['mi_name'];
    $form['mi_email'] = $_POST['mi_email'];
    $form['mi_phone'] = $_POST['mi_phone'];
    //$form['mi_subject'] = $_POST['mi_subject'];
    $form['mi_city'] = $_POST['mi_city'];
    $form['mi_country'] = $_POST['mi_country'];
    $form['mi_num_people'] = $_POST['mi_num_people'];
    ...
    //$message .= "Subject :" . $form['mi_subject']  ."\n";
    $message .= "Email : " . $form['mi_email']     ."\n";
    $message .= "Phone : " . $form['mi_phone']     ."\n";
    $message .= "City : " . $form['mi_city']     ."\n";
    $message .= "Nationality : " . $form['mi_country']     ."\n";
    $message .= "Number of persons : " . $form['mi_num_people']     ."\n"; 
    $message .= "Checkin : " . $form['mi_checkin']     ."\n";


    //set the form headers
    $headers = 'From: Podere San Cristoforo - Visits Request';

    // The email subject
    $subject = 'Mail richiesta visita';

    // Who are we going to send this form too
    $send_to = ['lorenzo.zonin@poderesancristoforo.it',$form['mi_email']];
...
add_action( 'wp_ajax_requestVisitMessage', 'request_visit_mail_form' );
add_action( 'wp_ajax_nopriv_requestVisitMessage', 'request_visit_mail_form' );